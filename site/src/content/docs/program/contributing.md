...
7. **Placement-against-element-law** (screen QA, added at the Teaching now bank 10-Jul): for every element on the screen, check its placement and behaviour against the owning element board's own law - a banked element used against its own rules (e.g. a banner placed low when the banner law says top/inline) is a composition slip even when every pixel of the element itself is correct.
8. **Field provenance map** (debuted at the Teaching now bank): screen sends for app screens trace every element to its backend endpoint + field with file:line evidence, verdict LIVE / DERIVED / GAP / CUT; the catalogue side sample-verifies citations in code before banking. Gaps feed the backend queue; honesty notes (async actions, display-only truths) bind the copy.

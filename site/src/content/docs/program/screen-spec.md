...
2. **Header** follows the canon (large-title size **30/700 FOUNDER-RATIFIED 13-Jul**: a smaller 24px was considered and declined so no screen drifts out of step - the size is a system-wide constant, never a per-screen choice): a root tab surface is a large left title 30/700 (+ optional 15/500 sub); a pushed/flow screen is a Phosphor back arrow + centred compact title **18/700**. The Teaching-now cockpit is the one sanctioned large/no-back exception; full-screen moments (camera, escalation recap) run headerless. *(Header canon, 11-Jul, measured 12-Jul.)*
...
10. **Bottom CTA dock = canvas action-zone + top hairline.** A pinned bottom dock (price + primary action, or any sticky CTA) sits on the canvas (`#EDF0F9`) with a **1px `#E7EBF0` top hairline** and safe-area bottom padding. No opaque fill, no drop shadow. *(Dock ruling, founder-ratified 13-Jul from the references corpus: 5 of 6 apps separate a pinned dock from content; flat detail pages use a hairline (Fresha), floating surfaces use shadow (Grab/Airbnb-over-map) — ours are flat, so hairline. A bare no-separator dock (inDrive) was the weakest-attested and is not used.)*
